<?php
    class repertoire {
        ...
        public function setRepertoireList($plusDays,$movieID) {
            if(!empty($_GET['localisation'])) {
                ...
                $connect = mysqli_connect("localhost", "ProjectCinema", "zaq1@WSX", "projectcinema");
                $query = 'SELECT DISTINCT `repertoire`.`movie_id` AS "unique_id" FROM `repertoire`, `movies`, `rooms` WHERE `repertoire`.`date` = "'.$today.'" AND `movies`.`id` = `repertoire`.`movie_id` AND `repertoire`.`localisation_id` = '.$localisation.' AND `repertoire`.`sold_fares` < `rooms`.`available_space` AND `repertoire`.`room_id` = `rooms`.`id` AND `movies`.`id` = "'.$movieID.'" ORDER BY `repertoire`.`time`;';
                $result = mysqli_query($connect,$query);
        ...
        private function setMovieTime($connect, $movie_id, $repertoireDate, $cinemaLocalisation) {
            ...
            while($row = mysqli_fetch_array($result)) {
                echo '<form action="../repertoire/tickets/index.php" method="post">' ;
                ...
                echo '<input type="submit" class="repertoire-time2 noselect" value="'.$row['2d/3d'].' | At: '.$row['time'].'"></option></select>'; 
                echo '</form>';
            }
